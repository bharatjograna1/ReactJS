...
function* doSellOrderData({payload}) { 
  ...
  const response = yield call(swaggerPostAPI,'api/Transaction/CreateTransactionOrderBG/' + pair,payload.sellOrderRequest,headers);   
...
function* doBuyOrderData({payload}) {
  ...
  var headers =  {'Authorization': AppConfig.authorizationToken}
  const response = yield call(swaggerPostAPI,'api/Transaction/CreateTransactionOrderBG/' + pair,payload.buyOrderRequest,headers);   
  //console.log(response)
