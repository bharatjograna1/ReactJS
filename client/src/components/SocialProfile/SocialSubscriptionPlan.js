        ...
        if(typeof nextProps.subscribeData !== 'undefined' && Object.keys(nextProps.subscribeData).length > 0) {
            if (nextProps.subscribeData.ReturnCode === 1 || nextProps.subscribeData.ReturnCode === 9) {
                ...
                if(this.state.type !== '' && this.state.type === 'Leader') {
                    ...
                    this.props.history.push({ pathname: '/app/social-profile/follower-profile-configuration', state : { profileId : this.state.profileId } });
                } else {
                    this.getSocialProfileList();
                }
            }
                                    ...
                                    <JbsCollapsibleCard customClasses="text-center border border-solid border-dark">
                                        ...
                                        <div className="col-md-12 mb-20">
                                            {list.Subscribe === false ?
                                                <Button disabled={ !list.Subscribe && disablePlan ? true : false } onClick={() => this.onSubscribe(list.ProfileId,list.ProfileType)} variant="raised" color="primary"><IntlMessages id={"sidebar.btnSubscribe"} /></Button>
                                                :
                                                <div className="row">
                                                    <div className="col-md-6 col-sm-12">
                                                        <Button disabled={ !list.Subscribe && disablePlan ? true : false } onClick={() => this.onUnsubscribe(list.ProfileId,list.ProfileType)} variant="raised" color="primary"><IntlMessages id={"sidebar.btnUnSubscribe"} /></Button>
                                                    </div>
                                                    <div className="col-md-6 col-sm-12">
                                        ...
                                        <div className="mt-10 text-left row">
                                            ...
                                            <div className="col-sm-2">{list.Profile_Visiblity}</div>
                                        </div>
                                        <div className="mt-10 text-left row">
                                            <div className="offset-sm-1 col-sm-7 pl-20">
                                                <IntlMessages id="socialProfile.canHaveFollower" />
                                            </div>
                                            <div className="col-sm-4">{list.Can_Have_Followers}</div>
                                        </div>
                                        <div className="mt-10 text-left row">
                                            <div className="offset-sm-1 col-sm-7 pl-20">
                                                <IntlMessages id="socialProfile.canFollowerLeaders" />
                                            </div>
                                            <div className="col-sm-2">{list.Can_Follow_Leaders}</div>
                                        </div>
                                        <div className="mt-10 text-left row">
                                            <div className="offset-sm-1 col-sm-7 pl-20">
                                                <IntlMessages id="socialProfile.canCopyTrade" />
                                            </div>
                                            <div className="col-sm-2">{list.Can_Copy_Trade}</div>
                                        </div>
                                        <div className="mt-10 text-left row">
                                            <div className="offset-sm-1 col-sm-7 pl-20">
                                                <IntlMessages id="socialProfile.canMirrorTrade" />
                                            </div>
                                            <div className="col-sm-2">{list.Can_Mirror_Trade}</div>
                                        </div>
                                        <div className="mt-10 text-left row">
                                            <div className="offset-sm-1 col-sm-7 pl-20">
                                                <IntlMessages id="socialProfile.minTradeVolume" />
                                            </div>
                                            <div className="col-sm-2">{list.Minimum_Trade_Volume}</div>
                                        </div>
                                    </JbsCollapsibleCard>
                                </div>
                            ))}
