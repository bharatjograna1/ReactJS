...
class BuyTrade extends Component {
  ...
  render() {
    ...
    const diffLimit = 6 - buyOrderDetail.length;
    // console.log("buyOrderDetail",buyOrderDetail);
    var buyOrderList = [];
    ...
    if(this.props.displayTable === false && diffLimit <= 6) {

      for(var lastIndex = buyOrderDetail.length;lastIndex < 6;lastIndex++) {
        buyOrderList.push(<BuyOrderRow
          key={lastIndex}        
          Price={"-"}
          Amount={"-"}        
    ...
    if(typeof firstPrice === 'undefined' || firstPrice === '' || firstPrice === null || firstPrice === '-') {
      ...
      <Fragment >   
        ...
        <div className="updownmarket">
          ...
          (this.props.displayTable ? "" : "upmarket")
            , "text-center fs-24")}>
