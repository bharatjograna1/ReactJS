...
// actions
import { collapsedSidebarAction, darkModeAction, changeThemeColor } from 'Actions';
...
class Header extends Component {
	...
	componentWillMount() {
		...
		// Added by Jayesh for google analytics on 28-12-2018 updated on 04-01-2019
		if(atob(localStorage.getItem("ganalytics")) !== 'undefined' && atob(localStorage.getItem("ganalytics")) !== null) 
		{
			try {

				let ganalytics = JSON.parse(atob(localStorage.getItem("ganalytics")));

				var re = /<script\b[^>]*>([\s\S]*?)<\/script>/gm;
				var matche;
				while (matche = re.exec(ganalytics.googleanalytics)) {
					const script = document.createElement("script");
					const scriptText = document.createTextNode(matche[1]);
					script.appendChild(scriptText);
					document.head.appendChild(script);
				}
